package com.shixincube.app.widget.recyclerview;
...
/**
 * RecyclerView（默认是竖起列表）
 * <p>
 * 不需要管理 LayoutManger，可以通过 xml 文件或 setter 方法设置
 */
public class RecyclerView extends androidx.recyclerview.widget.RecyclerView {
    ...
    public RecyclerView(Context context, AttributeSet attrs) {
        ...
        /*================== 获取自定义属性 Begin ==================*/
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RecyclerView);
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.RecyclerView_rv_type) {
                type = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.RecyclerView_rv_orientation) {
                orientation = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.RecyclerView_rv_column) {
                column = typedArray.getInt(attr, 1);
            } else if (attr == R.styleable.RecyclerView_rv_divider_size) {
                dividerSize = (int) typedArray.getDimension(attr, 0f);
            } else if (attr == R.styleable.RecyclerView_rv_divider_drawable) {
                dividerDrawable = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.RecyclerView_rv_divider_color) {
                dividerColor = typedArray.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.RecyclerView_rv_default_animator_open) {
    ...
    /**
     * 提醒 RecyclerView，当前的列表样式已经更改（一般是用代码动态修改了type和orientation后调用，如果修改了分割线样式，也需要调用该方法进行刷新）。
     */
    ...
    public void smoothMoveToPosition(int position) {
        ...
        if (position < 0 || position >= getAdapter().getItemCount()) {
            Log.e(RecyclerView.class.getName(), "Position overflow");
            return;
        }
        mIndex = position;
        stopScroll();
    ...
    /**
     * RecyclerView 的滚动监听, 用于平滑滚动条目置顶。
     */
    protected class RecyclerViewListener extends androidx.recyclerview.widget.RecyclerView.OnScrollListener {
        @Override
        public void onScrollStateChanged(androidx.recyclerview.widget.RecyclerView recyclerView, int newState) {
            ...
            if (move && newState == androidx.recyclerview.widget.RecyclerView.SCROLL_STATE_IDLE) {
                move = false;
                int n = mIndex - glm.findFirstVisibleItemPosition();
                if (0 <= n && n < RecyclerView.this.getChildCount()) {
                    int top = RecyclerView.this.getChildAt(n).getTop();
        ...
        public void onScrolled(androidx.recyclerview.widget.RecyclerView recyclerView, int dx, int dy) {
            ...
            if (move) {
                ...
                if (0 <= n && n < RecyclerView.this.getChildCount()) {
                    int top = RecyclerView.this.getChildAt(n).getTop();
                    RecyclerView.this.scrollBy(0, top);
                }
            }
        }
    }

    /*================== RecyclerView 的滚动事件拓展 Begin ==================*/
    ...
    /**
     * RecyclerView的滚动事件拓展（原滚动事件被用于平滑滚动）
     */
    public interface OnScrollListenerExtension {
        void onScrollStateChanged(androidx.recyclerview.widget.RecyclerView recyclerView, int newState);

        void onScrolled(androidx.recyclerview.widget.RecyclerView recyclerView, int dx, int dy);
    }
    
    /*================== RecyclerView 的滚动事件拓展 End ==================*/
     ...
     */
    protected class DefaultItemDecoration extends ItemDecoration {
        private Context mContext;
        ...
        public DefaultItemDecoration(Context context, int orientation, int dividerSize, int dividerColor, Drawable dividerDrawable) {
            mContext = context;
            ...
            if (dividerDrawable == null) {
                ...
                if (mDividerSize == 0) {
                    if (mOrientation == RecyclerView.ORIENTATION_VERTICAL) {
         ...
         */
        @Override
        public void onDraw(Canvas c, androidx.recyclerview.widget.RecyclerView parent, State state) {
            //纵向列表画横线，横向列表画竖线
            if (mOrientation == RecyclerView.ORIENTATION_VERTICAL) {
         ...
         */
        @Override
        public void getItemOffsets(Rect outRect, View view, androidx.recyclerview.widget.RecyclerView parent, State state) {
            if (mOrientation == RecyclerView.ORIENTATION_VERTICAL) {
         ...
         * 画横线
         */
        private void drawHorientationDivider(Canvas c, androidx.recyclerview.widget.RecyclerView parent, State state) {
            //得到分割线的四个点：左、上、右、下
            //画横线时左右可以根据parent得到
         ...
         * 画竖线
         */
        private void drawVerticalDivider(Canvas c, androidx.recyclerview.widget.RecyclerView parent, State state) {
            //画竖线时上下可以根据parent得到
